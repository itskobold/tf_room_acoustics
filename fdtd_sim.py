...
class FDTD:
    ...
    def run(self):
        ...
        grid_spacing = self.c / self.f_max / self.ppw
        dt = np.sqrt(0.5) * grid_spacing / self.c

        # Work out the dimensions of the FDTD grid
        # We add 2 to each spatial dimension to include boundary nodes
        self.x_len_samples = int(np.ceil(self.x_len / grid_spacing)) + 2
        self.y_len_samples = int(np.ceil(self.y_len / grid_spacing)) + 2
        self.td_len_samples = int(np.ceil(self.td_len / dt))

        # Init matrix (x, y, t)
        pressure = np.zeros([self.x_len_samples, self.y_len_samples, self.td_len_samples])
        ...
        for x_ in range(self.x_len_samples):
            for y_ in range(self.y_len_samples):
                x_val = x_ / self.x_len_samples * self.x_len
                y_val = y_ / self.y_len_samples * self.y_len
                x_val -= self.x_len / 2
                y_val -= self.y_len / 2
        ...
        # Constants for finite difference calculation
        lmb = np.sqrt(0.5)
        a = 2 - (4 * lmb ** 2)
        b = lmb ** 2
        c = -1
        ...
        # Save simulated FDTD data and metadata
        self.data = pressure
        self.metadata = self.get_metadata()

    # Return dictionary of metadata for the previous simulation
    def get_metadata(self):
        return {'x_len': self.x_len,
                'x_len_samples': self.x_len_samples,
                'y_len': self.y_len,
                ...
                'impulse_r': self.impulse_r,
                'impulse_a': self.impulse_a,
                'render_time_ms': self.computation_time}
